...
def delete_data(data: list[Vacancy], item_id: int) -> Union[list[Vacancy], int]:
...
def cast_obj_to_list(data: list[Vacancy]) -> list[dict]:
    """Преобразование списка вакансий в список словарей"""

    return [
        {"id": x.item_number, "name": x.name, "linq": x.linq, "salary_from": x.salary_from, "salary_to": x.salary_to}
        for x in data
    ]
